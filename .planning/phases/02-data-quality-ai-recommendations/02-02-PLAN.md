...
Output: Validation workflow with Glue DQ integration, freshness monitoring, alert system, and API endpoints.
</objective>
...
<task type="auto">
  ...
  <action>
    Create API endpoints for validation management:
  ...
  <verify>
    ...
    ls backend/api/validations/route.ts backend/api/rules/route.ts backend/api/alerts/route.ts
  </verify>
  <done>
    API endpoints compile successfully. Validations endpoint triggers Step Functions workflow. Rules endpoint supports CRUD with approval flow. Alerts endpoint allows status management.
  </done>
</task>
